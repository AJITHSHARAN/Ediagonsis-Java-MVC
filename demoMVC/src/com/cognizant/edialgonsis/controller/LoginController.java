...
public class LoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(result)
		{
			...
			List<Medicare_Services> medlist=bo.fetchdoc();
			List<Doctor> doclist=bo.fetchdoctor();
			session.setAttribute("customername", name);
			
			session.setAttribute("medlist", medlist);
			session.setAttribute("doclist", doclist);
			System.out.println(name);
			dispatcher=request.getRequestDispatcher("cuslanding.jsp");
